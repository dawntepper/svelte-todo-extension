<script lang="ts">
  ...
  function handleNewListKeyPress(e: KeyboardEvent) {
    handleEnterKey(e, editingListId ? saveEditedList : createNewList);
  }

  function handleEnterKey(e: KeyboardEvent, action: () => void) {
    if (e.key === "Enter") {
      e.preventDefault();
      action();
    }
  ...
  function handleNewTodoKeyPress(e: KeyboardEvent) {
  ...
  function handleBulkInputKeyPress(e: KeyboardEvent) {
